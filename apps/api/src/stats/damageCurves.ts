...
export interface DamageCurveGameRow {
  matchId: string;
  championId: number;
  championName: string;
  placement: number;
  timePlayedSeconds: number | null;
  /** `[t (ms), physical, magical, true]` per timeline frame, cumulative
   * damage to champions — `match_participants.frames` as stored. */
  frames: [number, number | null, number | null, number | null][];
}
 ...
 */
function gameCurve(frames: DamageCurveGameRow["frames"]): DamageCurvePoint[] {
...
type Game = { row: DamageCurveGameRow; points: DamageCurvePoint[] };
...
/** One row per tracked match with a stored timeline. */
export function buildDamageCurveStats(rows: readonly DamageCurveGameRow[]): DamageCurveStats {
  const games: Game[] = rows
    .map((row) => ({ row, points: gameCurve(row.frames) }))
