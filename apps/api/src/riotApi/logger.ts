...
import { REGION_LABEL, isPlatform, matchRegion, type Region } from "./routing.js";
 ...
 * every line carries both. Colors come from picocolors, which turns them off
 * when the output isn't a terminal (NO_COLOR / FORCE_COLOR are honored).
 */
...
export class RiotLogger {
  ...
  log(level: Exclude<LogLevel, "silent">, call: RiotCall, tag: number | string, details: Array<string | null>) {
    ...
    const line =
      `${pc.bold(coloredTag)} ${pc.dim("-")} ${pc.cyan(region)} ${pc.blue(platform)} ${pc.magenta(api)} ` +
