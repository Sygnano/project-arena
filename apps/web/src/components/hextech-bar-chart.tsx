...
function renderSegmentLabel(segment: BarSegment, minHeightForLabel: number) {
  return segment.label && segment.height >= minHeightForLabel ? (
    <span
      className="font-display text-[12px] leading-none text-[#0a1118]/85"
    >
      {segment.label}
    </span>
  ) : null;
...
function HextechBarChart({
  ...
  const isHighlighted = (column: BarColumn) =>
    highlightedId !== null && column.id === highlightedId;
  const columnOpacity = (column: BarColumn) =>
    column.dimmed && !isHighlighted(column) ? 0.4 : undefined;
  const columnFilter = (column: BarColumn) =>
    isHighlighted(column) ? HIGHLIGHT_FILTER : undefined;
        ...
        >
          ...
          {columns.map((column, index) => {
            ...
            const columnStyle = {
              ...
              filter:
                selectionStyle === "brightness" ? columnFilter(column) : undefined,
            ...
            if (isPercent) {
              ...
              let runningBottom = 0;
              const segmentBottoms = new Array<number>(
                column.segments.length,
              );
