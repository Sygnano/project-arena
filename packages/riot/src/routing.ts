 ...
 * goes to is also the key of its rate limits and of its priority queue at the
 * Riot gateway (apps/riot-gateway).
 *
...
export function toPlatform(value: string): Platform {
  ...
  if (!isPlatform(platform))
    throw new Error(`Unknown Riot platform "${value}": add it to PLATFORMS in packages/riot/src/routing.ts`);
  return platform;
}
